...
contract AccountAbstraction is IAccount ,Ownable{
    ...
    ) external override requireByEntryPoint returns (uint256 validationData) {
    ...
    ) public requireByEntryPointOrOwner {
        // which mena the address will have the fallback function and receive function
        (bool success, bytes memory data) = dest.call{value: _amount}(
            functionCall
        );
        if (!success) {
